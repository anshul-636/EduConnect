...
export default function Attendance() {
  ...
  const isStaff = user?.role === 'TEACHER' || user?.role === 'SCHOOL' || user?.role === 'ADMIN';

  return isStaff ? <StaffAttendanceView /> : <StudentAttendanceView />;
}

/* ───────────────────────────── Student view ──────────────────────────────
   Students can only see their OWN attendance (the backend blocks them from
   the full class roster now), so this hits /attendance/:classId/my-report. */
function StudentAttendanceView() {
  ...
  const loadClasses = async () => {
    ...
    try {
      ...
      setClasses(list);
      if (list.length) { setSelectedClass(list[0].id); loadReport(list[0].id); }
  ...
  const loadReport = async (classId) => {
    setLoading(true);
    try {
      const r = await attendanceService.getMyReport(classId);
      setReport(r.data.data);
    } catch (_) { setReport(null); }
    setLoading(false);
  };
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-dark-300 font-semibold">No attendance records yet</p>
            <p className="text-dark-500 text-sm mt-1">Your teacher hasn't marked attendance for this class yet.</p>
          </div>
        ) : (
          <>
            {/* Summary cards */}
            <div className="grid grid-cols-4 gap-3 mb-6">
              {[
                { label: 'Total', val: report.total, color: 'text-dark-200', bg: 'bg-dark-800' },
            ...
            <div className="flex items-center gap-3 mb-6 px-4 py-3 bg-dark-800 border border-dark-700 rounded-2xl reveal delay-2">
              ...
              <span className={`text-sm font-bold ${report.percentage >= 75 ? 'text-emerald-300' : report.percentage >= 50 ? 'text-amber-300' : 'text-red-300'}`}>
...
/* ──────────────────────────── Teacher/School view ─────────────────────────
   Unchanged from before — mark daily attendance, view the monthly roster. */
                  ...
                  return (
                    <div key={student.id}
                      ...
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${cfg.dot}`} />
                        <span className="text-xs font-semibold">{cfg.label}</span>
                        <span className="text-dark-600 text-[10px]">click to cycle</span>
                      </div>
                    </div>
                  );
                })}
