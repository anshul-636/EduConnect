...
const getAll = async (req, res) => {
  try {
    const posts = await forumService.getAll(req.query);
...
const reply = async (req, res) => {
  if (!req.body.content) return res.status(422).json({ success: false, message: 'Content is required.' });
  try {
...
module.exports = { create, getAll, getById, reply, remove };
