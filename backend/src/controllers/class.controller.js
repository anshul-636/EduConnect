...
exports.addSlot = async (req, res) => { try { ok(res, await svc.addTimetableSlot(req.params.id, req.body, req.user.id, req.user.role), 201); } catch (e) { err(res, e); } };
exports.updateSlot = async (req, res) => { try { ok(res, await svc.updateTimetableSlot(req.params.slotId, req.body, req.user.id, req.user.role)); } catch (e) { err(res, e); } };
exports.deleteSlot = async (req, res) => { try { await svc.deleteTimetableSlot(req.params.slotId, req.user.id, req.user.role); ok(res, { deleted: true }); } catch (e) { err(res, e); } };
